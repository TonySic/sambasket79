...
<div class="container">
				...
				@foreach (session("panier") as $key => $item)
					...
					<tr>
						...
						<td>
							...
							<form method="get" action="{{ route('panier', $key) }}"> @csrf
								<input type="number" nom="quantite" placeholder="Quantité ?" value="{{ $item['quantite'] }}" class="form-control" style="width: 80px" >
						...
						<td>
							...
							<a href="{{ route('panier.remove', $key) }}" class="btn boutonRouge btn-sm" title="Retirer le produit du panier" >Retirer</a>
						</td>
					</tr>
				@endforeach
				<tr colspan="2" >
	...
	<a class="btn boutonRouge" href="{{ route('panier.empty') }}" title="Retirer tous les produits du panier" >Vider le panier</a>
